...
if not ffi_ok then
    warn("ERROR: LuaJIT ffi not available, this script cannot run.")
end

local PLATFORM = ffi_ok and ffi.os or "Unknown"
local backend  = nil
...
local function init_windows()
    ...
    local ok1, user32 = pcall(ffi.load, "user32")
    local ok2, kernel32 = pcall(ffi.load, "kernel32")
    if not ok1 then return nil end

    return function()
        local hwnd = user32.GetForegroundWindow()
        if hwnd == nil then return nil end
        if ignore_obs_window and ok2 then
...
local function init_linux()
    ...
    if not x11 then
        warn("ERROR: Could not load libX11. (If you're on Wayland, see the note below.)")
        return nil
    end

    local dpy = x11.XOpenDisplay(nil)
    if dpy == nil then
        warn("ERROR: Could not connect to the X server (no DISPLAY?).")
        return nil
    end
    local root = x11.XDefaultRootWindow(dpy)

    local noop = ffi.cast("XErrorHandlerFn", function(d, e) return 0 end)

    local function get_card_prop(win, atom_name, want_items)
        ...
        if x11.XGetWindowProperty(dpy, win, a, 0, want_items, 0, 0, at, af, ni, ba, pr) ~= 0 then
            return nil
        end
        local val = nil
    ...
    return function()
        local prev = x11.XSetErrorHandler(noop)
        local cx, cy

        repeat
            local win = get_card_prop(root, "_NET_ACTIVE_WINDOW", 1)
            if win == nil or win == 0 then break end

            if ignore_obs_window then
                local pid = get_card_prop(win, "_NET_WM_PID", 1)
...
local SETTING_KEYS = { "monitor_id", "monitor", "screen_id", "screen", "display" }
    ...
    if sources ~= nil then
        for _, src in ipairs(sources) do
            ...
            elseif id == "pipewire-desktop-capture-source" then
                warn("PipeWire source found; the API can't tell which screen it captures, skipping.")
            end
        end
        obs.source_list_release(sources)
    end
    if #rects == 0 then warn("No matchable screen capture source found.") end
end
...
local function find_source(px, py)
    ...
    local best, bd
    for _, r in ipairs(rects) do
        local dx = px - (r.x + r.w / 2)
        local dy = py - (r.y + r.h / 2)
        local d = dx * dx + dy * dy
...
local function tick()
    if not enabled or backend == nil then return end
    local px, py = backend()
...
local function restart_timer()
    ...
    if enabled and backend ~= nil then
        obs.timer_add(tick, poll_ms)
        timer_on = true
    end
end
...
function script_description()
    return [[<b>Auto Monitor Switch</b> (Windows + Linux/X11)<br/>
Odaktaki pencere hangi ekrandaysa, o ekrani yakalayan kaynagi otomatik gorunur
yapar, digerlerini gizler. Sahne ve kaynak adlari otomatik bulunur.<br/><br/>
<small>Linux'ta <i>Screen Capture (XSHM)</i> kaynaklari gerekir; PipeWire
kaynaklari eslestirilemez.</small>]]
end
...
function script_properties()
    ...
    obs.obs_properties_add_button(p, "rescan", "Kaynaklari yeniden tara", function()
        rebuild_index()
        last_name = nil
        warn("Platform: " .. PLATFORM .. ", sources found: " .. #rects)
        for _, r in ipairs(rects) do
...
function script_load(s)
    ...
    if PLATFORM == "Windows" then
        backend = init_windows()
    elseif PLATFORM == "Linux" then
        if os.getenv("WAYLAND_DISPLAY") then
            warn("WARNING: Wayland session detected. Only XWayland windows are visible; "
                 .. "an X11 session is recommended for reliable operation.")
        end
        backend = init_linux()
    else
        warn("Window tracking is not implemented for this platform: " .. PLATFORM)
    end
    if backend == nil then
        warn("Could not start window tracking, script inactive.")
        return
    end
